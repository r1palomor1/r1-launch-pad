...
const playerFavoriteBtn_playlist = document.getElementById('playerFavoriteBtn_playlist');
...
const playerHomeNavBtn = document.getElementById('playerHomeNavBtn');
const playerMusicNavBtn = document.getElementById('playerMusicNavBtn');
const playerPlaylistsNavBtn = document.getElementById('playerPlaylistsNavBtn');
const playerFavoritesNavBtn = document.getElementById('playerFavoritesNavBtn');
...
const PLAY_ICON_SVG = `<svg xmlns="http://www.w3.org/2000/svg" width="24px" height="24px" viewBox="0 0 24 24" fill="none" class="icon line-color" data-name="Line Color"><path d="M8 5v14l11-7z" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" /></svg>`;
const PAUSE_ICON_SVG = `<svg xmlns="http://www.w3.org/2000/svg" width="24px" height="24px" viewBox="0 0 24 24" fill="none" class="icon line-color" data-name="Line Color"><path d="M6 19h4V5H6v14zm8-14v14h4V5h-4z" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" /></svg>`;
const STOP_ICON_SVG = `<svg xmlns="http://www.w3.org/2000/svg" width="24px" height="24px" viewBox="0 0 24 24" fill="none" class="icon line-color" data-name="Line Color"><path d="M6 6h12v12H6z" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" /></svg>`;
const AUDIO_ICON_SVG = `<svg xmlns="http://www.w3.org/2000/svg" width="24px" height="24px" viewBox="0 0 24 24" fill="none" class="icon line-color" data-name="Line Color"><path d="M12 3a9 9 0 0 0-9 9v7c0 1.1.9 2 2 2h4v-8H5v-1a7 7 0 0 1 14 0v1h-4v8h4c1.1 0 2-.9 2-2v-7a9 9 0 0 0-9-9z" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" /></svg>`;

// Heart icon for Save/Unsave function
const HEART_ICON_SVG = `<svg xmlns="http://www.w3.org/2000/svg" width="24px" height="24px" viewBox="0 0 24 24" fill="none" class="icon line-color" data-name="Line Color"><path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" /></svg>`;

// Volume Control Icons
const VOLUME_HIGH_ICON_SVG = `<svg xmlns="http://www.w3.org/2000/svg" width="24px" height="24px" viewBox="0 0 24 24" fill="none" class="icon line-color" data-name="Line Color"><path d="M3 9v6h4l5 5V4L7 9H3zm13.5 3A4.5 4.5 0 0 0 14 7.97v8.05c1.48-.73 2.5-2.25 2.5-4.02zM14 3.23v2.06c2.89.86 5 3.54 5 6.71s-2.11 5.85-5 6.71v2.06c4.01-.91 7-4.49 7-8.77s-2.99-7.86-7-8.77z" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1" ></path></svg>`;
const VOLUME_MUTED_ICON_SVG = `<svg xmlns="http://www.w3.org/2000/svg" width="24px" height="24px" viewBox="0 0 24 24" fill="none" class="icon line-color" data-name="Line Color"><path d="M16.5 12A4.5 4.5 0 0 0 14 7.97v2.21l2.45 2.45c.03-.2.05-.41.05-.63zm2.5 0c0 .94-.2 1.82-.54 2.64l1.51 1.51A8.796 8.796 0 0 0 21 12c0-4.28-2.99-7.86-7-8.77v2.06c2.89.86 5 3.54 5 6.71s-2.11 5.85-5 6.71zM4.27 3L3 4.27 7.73 9H3v6h4l5 5v-6.73l4.25 4.25c-.67.52-1.42.93-2.25 1.18v2.06a8.99 8.99 0 0 0 3.69-1.81L19.73 21 21 19.73l-9-9L4.27 3zM12 4L9.91 6.09 12 8.18V4z" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1" ></path></svg>`;
    ...
    searchHandler(searchInput.value); // This still runs to update the category star
        ...
        if (isYouTube) {
            ...
            if (playlistId) {
                ...
                // Generic or channel link → open player with empty state
                openPlayerView({ title: link.description || 'YouTube' });
            }
            // --- END OF NEW LOGIC ---
        } else {
...
function hidePlayerUI() {
    ...
    // Favorite button is now always visible in Song mode bottom controls
    // ⬆️ *** END OF MODIFIED CODE *** ⬆️
...
function showPlayerUI() {
    ...
    // Favorite button is now in Song mode bottom controls, no need to hide
    // ⬆️ *** END OF MODIFIED CODE *** ⬆️
...
(function wireUiHideTimerResets() {
    const ids = [
        // Songs mode
        'playerPlayPauseBtn','playerAudioOnlyBtn',

        // Playlist mode
        'playerPlayPauseBtn_playlist','playerAudioOnlyBtn_playlist',
        'playerPrevBtn','playerNextBtn',
       
        // Shared/overlay bits
        'playerPlaylistBtn','stopPlayingBtn'
    ];
...
// === REDESIGN: Media Discovery Function ===
/**
 * Opens the search overlay with the specified mode
 * @param {string} mode - 'videos', 'playlists', or 'is.gd'
 */
function openMediaDiscovery(mode) {
    const isAlreadyOpen = youtubeSearchViewOverlay.style.display === 'flex';
    const isSameMode = currentSearchMode === mode;
    
    // If clicking the same icon when overlay is open, toggle header visibility
    if (isAlreadyOpen && isSameMode) {
        // Toggle header visibility
        if (isSearchHeaderCollapsed) {
            toggleSearchHeader(true); // Show header
        } else {
    ...
    // Restore cached results based on mode (independent for each mode)
    const hasCachedResults = (mode === 'videos' && videosResults.html) || 
                             (mode === 'playlists' && playlistsResults.html);
    
    if (mode === 'videos' && videosResults.html) {
    ...
    } else if (mode === 'playlists' && playlistsResults.html) {
    ...
    // Header visibility: Hide if we have cached results, show if empty
    if (hasCachedResults || mode === 'is.gd') {
        toggleSearchHeader(false); // Hide header - user can click icon again to show
    } else {
        toggleSearchHeader(true); // Show header for new searches
    }
    
    // Focus the search input only if header is visible
    if (!isSearchHeaderCollapsed) {
...
function renderYouTubeResults(results, mode) {
    ...
    results.forEach((item, index) => {
        ...
        // Apply is-favorite class after innerHTML is set (like landing page does)
        if (isSaved) {
            itemCard.querySelector('.add-favorite-btn')?.classList.add('is-favorite');
        }
...
function toggleSearchHeader(show) {
    if (show) {
        ...
        // Hide search toggle button when header is visible
        if (playerSearchToggleBtn) {
            playerSearchToggleBtn.style.display = 'none';
        }
        ...
        // Show search toggle button when header is hidden
        if (playerSearchToggleBtn) {
            playerSearchToggleBtn.style.display = 'block';
        }
    }
}
// === END OF NEW FUNCTION ===
...
// === NEW: Scroll listener for pagination only (header hiding removed) ===
youtubeSearchView.addEventListener('scroll', () => {
    // This listener handles infinite scroll pagination only
        ...
        if (li) {
            ...
            if (isYouTube) {
                ...
                if (playlistId) {
                    ...
                    // Generic or channel link → open player with empty state
                    openPlayerView({ title: name || 'YouTube' });
                }
                // --- END OF NEW LOGIC ---
            } else {
...
function returnToSearchFromPlayer(focusInput = false) {
    ...
    if (favoriteBtn) {
        ...
        // Item is not saved, add it - UPDATE UI FIRST for instant feedback
    ...
    if (card) {
        ...
        // 3. Update favorite button status for both modes
        const isSaved = savedPlaylists.some(p => p.id === cardId);
        [playerFavoriteBtn, playerFavoriteBtn_playlist].forEach(btn => {
            if (btn) {
                btn.classList.toggle('is-favorite', isSaved);
    ...
    // --- ⬇️ SHARED SEARCH TRIGGER FUNCTION ⬇️ ---
    const performSearch = async () => {
        const query = youtubeSearchInput.value.trim();
        if (!query) return; // Do nothing if input is empty
    
        // Check if search term is new or repeated
        ...
        // Hide the header after search is initiated
        ...
        if (currentSearchMode === 'videos') {
            ...
            if (!resolvedUrl) {
                ...
                toggleSearchHeader(true); // Show input again on error
                return;
            }
            ...
            if (savedPlaylists.some(p => p.id === playlistId)) {
                ...
                setTimeout(() => {
                    renderSavedPlaylists();
                    toggleSearchHeader(true); // Show input again
                }, 2000);
                youtubeSearchInput.value = '';
                return;
            }
            ...
            if (metadata) {
                ...
                renderSavedPlaylists();
                youtubeSearchInput.value = '';
                toggleSearchHeader(true); // Show input again after adding
            } else {
...
searchModeIsGdBtn.addEventListener('click', () => {
    ...
    // Show the search view if it's not already visible
...
// Duplicate favorite button listener for playlist mode
playerFavoriteBtn_playlist.addEventListener('click', async (e) => {
    // ⬇️ *** THE CRITICAL FIX: Stop the click from bubbling up *** ⬇️
    e.stopPropagation(); 
    // ⬆️ *** END OF CRITICAL FIX *** ⬆️
    ...
    const favoriteBtn = playerFavoriteBtn_playlist;
    const index = savedPlaylists.findIndex(p => p.id === itemId);
...
// ⬆️ *** END OF NEW LISTENER *** ⬆️
    ...
    // Initialize the page after all async operations complete
    renderLinks();
...
function setFocusOnCurrentlyPlaying(mode) {
    ...
    if (currentlyPlayingCardMode === mode && currentlyPlayingCardId) {
        ...
        // ⬇️ FIX: Check save status and update the player's Favorite button fill ⬇️
        const isSaved = savedPlaylists.some(p => p.id === currentlyPlayingCardId);
        [playerFavoriteBtn, playerFavoriteBtn_playlist].forEach(btn => {
            if (btn) {
...
// === REDESIGN: Navigation Button Event Listeners ===
// Home Button - Two-step navigation
if (playerHomeNavBtn) {
    playerHomeNavBtn.addEventListener('click', () => {
        const overlayVisible = youtubeSearchViewOverlay.style.display === 'flex';
        const playerVisible = internalPlayerOverlay.style.display === 'flex';
        
        if (overlayVisible) {
            // Step 1: Close search overlay
            closeYouTubeSearchView();
        } else if (playerVisible) {
            // Step 2: Close player and go back to main landing page
            internalPlayerOverlay.style.display = 'none';
            goHome();
        }
    });
...
// Music Button - Opens Videos mode
if (playerMusicNavBtn) {
    playerMusicNavBtn.addEventListener('click', () => {
        openMediaDiscovery('videos');
    });
}

// Playlists Button - Opens Playlists mode
if (playerPlaylistsNavBtn) {
    playerPlaylistsNavBtn.addEventListener('click', () => {
        openMediaDiscovery('playlists');
    });
}

// Favorites Button - Opens Saved Content mode
if (playerFavoritesNavBtn) {
    playerFavoritesNavBtn.addEventListener('click', () => {
        openMediaDiscovery('is.gd');
    });
}

// Search Toggle Button - Shows search header when hidden
if (playerSearchToggleBtn) {
